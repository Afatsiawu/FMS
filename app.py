...
def dashboard_report():
    ...
    return jsonify({
        ...
        'monthlyTotal': monthly_income,
        'monthlyBalance': monthly_income - monthly_expense
    })

@app.route('/api/reports/pl-statement')
def pl_statement():
    conn = sqlite3.connect('church_management.db')
    cursor = conn.cursor()
    
    # Local Assembly Income (23% of offerings/tithes + other income)
    cursor.execute('''
        SELECT COALESCE(SUM(COALESCE(local_amount, amount)), 0) 
        FROM income
    ''')
    total_local_income = cursor.fetchone()[0]
    
    # District Contribution Expense (77% of offerings/tithes)
    cursor.execute('''
        SELECT COALESCE(SUM(district_amount), 0) 
        FROM district_expenses
    ''')
    total_district_expense = cursor.fetchone()[0]
    
    # Other Expenses
    cursor.execute('''
        SELECT COALESCE(SUM(amount), 0) 
        FROM expenses WHERE expense_type = 'other'
    ''')
    total_other_expense = cursor.fetchone()[0]
    
    net_profit_loss = total_local_income - total_district_expense - total_other_expense
    
    conn.close()
    
    return jsonify({
        'localIncome': total_local_income,
        'districtExpense': total_district_expense,
        'otherExpense': total_other_expense,
...
def export_data():
    ...
    tables_info = {
        ...
        'tithes': 'Individual Tithes',
        'expenses': 'Expenses',
        'district_expenses': 'District Expenses',
        'inventory': 'Inventory',
        'accounting_entries': 'Accounting Entries'
    }
    
    for table_name, display_name in tables_info.items():
        # Create worksheet
        ...
        if records:
            ...
            for row_num, record in enumerate(records, 2):
                for col_num, value in enumerate(record, 1):
                    ...
                    cell.value = value
                    if col_num > 1:  # Don't center ID column
                        cell.alignment = center_alignment
            ...
            # Add header for empty table
            ws.cell(row=1, column=1, value="No data available")
    
    # Create summary sheet
    summary_ws = wb.create_sheet(title="Summary Report", index=0)
    
    # Church header
    summary_ws.merge_cells('A1:F1')
    header_cell = summary_ws.cell(row=1, column=1)
    header_cell.value = "THE APOSTOLIC CHURCH GHANA - ATTA NE ATTA"
    header_cell.font = Font(bold=True, size=16, color="1E3C72")
    header_cell.alignment = center_alignment
    
    summary_ws.merge_cells('A2:F2')
    subheader_cell = summary_ws.cell(row=2, column=1)
    subheader_cell.value = "FINANCIAL MANAGEMENT SYSTEM REPORT"
    subheader_cell.font = Font(bold=True, size=12)
    subheader_cell.alignment = center_alignment
    
    summary_ws.merge_cells('A3:F3')
    date_cell = summary_ws.cell(row=3, column=1)
    date_cell.value = f"Generated on: {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}"
    date_cell.alignment = center_alignment
    
    # Financial summary
    row = 5
    summary_ws.cell(row=row, column=1, value="FINANCIAL SUMMARY").font = Font(bold=True, size=14)
    row += 2
    
    # Calculate totals
    cursor.execute('SELECT COALESCE(SUM(COALESCE(local_amount, amount)), 0) FROM income')
    total_income = cursor.fetchone()[0]
    
    cursor.execute('SELECT COALESCE(SUM(amount), 0) FROM expenses')
    total_expenses = cursor.fetchone()[0]
    
    cursor.execute('SELECT COALESCE(SUM(district_amount), 0) FROM district_expenses')
    district_expenses = cursor.fetchone()[0]
    
    net_balance = total_income - total_expenses - district_expenses
    
    # Add summary data
    summary_data = [
        ("Total Local Income (23% + Others)", f"₵{total_income:,.2f}"),
        ("Total Local Expenses", f"₵{total_expenses:,.2f}"),
        ("District Contributions (77%)", f"₵{district_expenses:,.2f}"),
    ...
    for label, value in summary_data:
        ...
        if "Net Balance" in label:
            color = "008000" if net_balance >= 0 else "FF0000"
            summary_ws.cell(row=row, column=2).font = Font(bold=True, color=color)
        row += 1
    
    # Record counts
    row += 2
    summary_ws.cell(row=row, column=1, value="RECORD COUNTS").font = Font(bold=True, size=14)
    row += 2
    
    for table_name, display_name in tables_info.items():
        cursor.execute(f'SELECT COUNT(*) FROM {table_name}')
        count = cursor.fetchone()[0]
        summary_ws.cell(row=row, column=1, value=display_name).font = Font(bold=True)
    ...
    filename = f"Church_Management_Report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
...
def export_pdf():
    ...
    # Church header
    title = Paragraph("THE APOSTOLIC CHURCH GHANA - ATTA NE ATTA", title_style)
    subtitle = Paragraph("COMPREHENSIVE FINANCIAL REPORT", styles['Heading3'])
    date_para = Paragraph(f"Generated: {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}", styles['Normal'])
    ...
    # Create Excel-style layout with 2x3 grid
    # Get all data
    cursor.execute('SELECT date, category, description, amount, COALESCE(local_amount, amount) FROM income ORDER BY date DESC')
    income_records = cursor.fetchall()
    
    cursor.execute('SELECT member_name, member_phone, month, week1, week2, week3, week4, week5 FROM tithes ORDER BY month DESC, member_name')
    tithe_records = cursor.fetchall()
    
    cursor.execute('SELECT date, category, description, amount FROM expenses ORDER BY date DESC')
    expense_records = cursor.fetchall()
    
    cursor.execute('SELECT date, source, original_amount, district_amount FROM district_expenses ORDER BY date DESC')
    district_records = cursor.fetchall()
    
    cursor.execute('SELECT date, category, description, amount FROM expenses WHERE expense_type = "national" ORDER BY date DESC')
    national_records = cursor.fetchall()
    
    # Define table style for all sections
    ...
    # TOP ROW: Income (left) and Expenses (right)
    top_row_data = []
    
    # Income section (top-left)
    income_section = [Paragraph("INCOME TRANSACTIONS", section_style)]
    if income_records:
        income_data = [['Date', 'Category', 'Description', 'Total', 'Local']]
        for record in income_records[:15]:  # Limit to 15 records
            income_data.append([
                datetime.strptime(record[0], '%Y-%m-%d').strftime('%d/%m/%y'),
                record[1][:12], record[2][:15], f'₵{record[3]:,.0f}', f'₵{record[4]:,.0f}'
            ])
        
        income_table = Table(income_data, colWidths=[0.7*inch, 0.9*inch, 1.1*inch, 0.7*inch, 0.7*inch])
        income_table.setStyle(table_style)
        income_section.append(income_table)
    else:
        income_section.append(Paragraph("No income records", styles['Normal']))
    
    # Expenses section (top-right)
    expense_section = [Paragraph("LOCAL EXPENSES", section_style)]
    if expense_records:
        expense_data = [['Date', 'Category', 'Description', 'Amount']]
        for record in expense_records[:15]:  # Limit to 15 records
            expense_data.append([
                datetime.strptime(record[0], '%Y-%m-%d').strftime('%d/%m/%y'),
                record[1][:12], record[2][:18], f'₵{record[3]:,.0f}'
            ])
        
        expense_table = Table(expense_data, colWidths=[0.7*inch, 1*inch, 1.3*inch, 0.8*inch])
        expense_table.setStyle(table_style)
        expense_section.append(expense_table)
    else:
        expense_section.append(Paragraph("No expense records", styles['Normal']))
    
    # Create side-by-side layout for top row
    top_table_data = [[income_section, expense_section]]
    top_layout = Table(top_table_data, colWidths=[4.1*inch, 3.8*inch])
    ...
    # MIDDLE ROW: Individual Tithes (left) and District Expenses (right)
    # Individual Tithes section (bottom-left)
    tithe_section = [Paragraph("INDIVIDUAL TITHES", section_style)]
    if tithe_records:
        tithe_data = [['Member', 'Phone', 'Month', 'W1', 'W2', 'W3', 'W4', 'Total']]
        for record in tithe_records[:12]:  # Limit to 12 records
            total = (record[3] or 0) + (record[4] or 0) + (record[5] or 0) + (record[6] or 0) + (record[7] or 0)
            month_name = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 
                         'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'][record[2]]
            tithe_data.append([
                record[0][:10], record[1][:10], month_name,
                f'₵{record[3] or 0:.0f}', f'₵{record[4] or 0:.0f}', 
                f'₵{record[5] or 0:.0f}', f'₵{record[6] or 0:.0f}', f'₵{total:.0f}'
            ])
        
        tithe_table = Table(tithe_data, colWidths=[0.8*inch, 0.7*inch, 0.4*inch, 0.4*inch, 0.4*inch, 0.4*inch, 0.4*inch, 0.5*inch])
        tithe_table.setStyle(table_style)
        tithe_section.append(tithe_table)
    else:
        tithe_section.append(Paragraph("No tithe records", styles['Normal']))
    
    # District Expenses section (middle-right)
    district_section = [Paragraph("DISTRICT EXPENSES (77%)", section_style)]
    if district_records:
        district_data = [['Date', 'Source', 'Original', 'District']]
        for record in district_records[:12]:  # Limit to 12 records
            district_data.append([
                datetime.strptime(record[0], '%Y-%m-%d').strftime('%d/%m/%y'),
                record[1][:15], f'₵{record[2]:,.0f}', f'₵{record[3]:,.0f}'
            ])
        
        district_table = Table(district_data, colWidths=[0.7*inch, 1.2*inch, 0.8*inch, 0.8*inch])
        district_table.setStyle(table_style)
        district_section.append(district_table)
    else:
        district_section.append(Paragraph("No district expense records", styles['Normal']))
    
    # Create side-by-side layout for middle row
    middle_table_data = [[tithe_section, district_section]]
    middle_layout = Table(middle_table_data, colWidths=[4.1*inch, 3.8*inch])
    middle_layout.setStyle(TableStyle([
        ('VALIGN', (0, 0), (-1, -1), 'TOP'),
        ('LEFTPADDING', (0, 0), (-1, -1), 0),
        ('RIGHTPADDING', (0, 0), (-1, -1), 0),
        ('TOPPADDING', (0, 0), (-1, -1), 0),
    ...
    elements.append(middle_layout)
    elements.append(Spacer(1, 15))
    
    # BOTTOM: National Expenses (right side)
    national_section = [Paragraph("NATIONAL EXPENSES", section_style)]
    if national_records:
        national_data = [['Date', 'Category', 'Description', 'Amount']]
        for record in national_records[:10]:  # Limit to 10 records
            national_data.append([
                datetime.strptime(record[0], '%Y-%m-%d').strftime('%d/%m/%y'),
                record[1][:12], record[2][:18], f'₵{record[3]:,.0f}'
            ])
        
        national_table = Table(national_data, colWidths=[0.7*inch, 1*inch, 1.3*inch, 0.8*inch])
        national_table.setStyle(table_style)
        national_section.append(national_table)
    else:
        national_section.append(Paragraph("No national expense records", styles['Normal']))
    
    # Create layout for bottom section (right-aligned)
    bottom_table_data = [["", national_section]]
    bottom_layout = Table(bottom_table_data, colWidths=[4.1*inch, 3.8*inch])
    ...
    summary_data = [
        ['FINANCIAL SUMMARY', ''],
        ['Total Local Income', f'₵{total_income:,.2f}'],
        ['Total Local Expenses', f'₵{total_expenses:,.2f}'],
        ['District Contributions', f'₵{district_expenses:,.2f}'],
        ['NET PROFIT/LOSS', f'₵{net_balance:,.2f}']
    ]
    ...
    summary_table.setStyle(TableStyle([
        ...
        ('BACKGROUND', (0, -1), (-1, -1), colors.HexColor('#90EE90') if net_balance >= 0 else colors.HexColor('#FFB6C1')),
    ...
    elements.append(Paragraph("Pastor Kwame Asante | P.O. Box 1234, Accra, Ghana | +233 24 123 4567", footer_style))
    ...
    buffer.seek(0)
    filename = f"church_financial_report_{date.today().strftime('%Y%m%d')}.pdf"
    
    return send_file(buffer, as_attachment=True, download_name=filename, mimetype='application/pdf')

@app.route('/api/year-reset', methods=['POST'])
def year_reset():
    conn = None
    try:
        conn = sqlite3.connect('church_management.db', timeout=20.0)
        ...
        # Simple approach - just clear the data without archiving for now
        cursor.execute('DELETE FROM income')
        cursor.execute('DELETE FROM expenses') 
        cursor.execute('DELETE FROM tithes')
        cursor.execute('DELETE FROM district_expenses')
        cursor.execute('DELETE FROM accounting_entries')
        ...
        return jsonify({
            'success': True, 
            'message': f'Year {current_year} data cleared and system reset for new year'
        })
...
def get_historical_years():
    try:
        ...
        cursor.execute('SELECT DISTINCT year FROM historical_income ORDER BY year DESC')
...
@app.route('/api/historical-data/<int:year>')
def get_historical_data(year):
    try:
        ...
        # Get P&L data
        cursor.execute('SELECT category, SUM(local_amount) FROM historical_income WHERE year = ? GROUP BY category', (year,))
        revenue_data = {row[0]: row[1] for row in cursor.fetchall()}
        
        cursor.execute('SELECT category, SUM(amount) FROM historical_expenses WHERE year = ? GROUP BY category', (year,))
        expense_data = {row[0]: row[1] for row in cursor.fetchall()}
        
        cursor.execute('SELECT SUM(district_amount) FROM historical_district_expenses WHERE year = ?', (year,))
        district_total = cursor.fetchone()[0] or 0
        if district_total > 0:
            expense_data['District Contributions (77%)'] = district_total
        
        # Get transactions
        cursor.execute('''
            SELECT date, 'Income' as type, category, description, local_amount as amount 
            FROM historical_income WHERE year = ?
            UNION ALL
            SELECT date, 'Expense' as type, category, description, amount 
            FROM historical_expenses WHERE year = ?
            UNION ALL
            SELECT date, 'District Expense' as type, 'District Contribution' as category, source as description, district_amount as amount 
            FROM historical_district_expenses WHERE year = ?
            ORDER BY date DESC
        ''', (year, year, year))
        ...
        return jsonify({'pl': {'revenue': {}, 'expenses': {}}, 'transactions': []})
